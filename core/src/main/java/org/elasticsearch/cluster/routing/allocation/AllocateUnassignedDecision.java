...
public class AllocateUnassignedDecision implements ToXContent, Writeable {
    ...
    public static final AllocateUnassignedDecision NOT_TAKEN =
        new AllocateUnassignedDecision(null, null, null, null, null, null, false, false);
    /**
    ...
    static {
        ...
        cachedDecisions.put(AllocationStatus.FETCHING_SHARD_DATA,
            new AllocateUnassignedDecision(Type.NO, AllocationStatus.FETCHING_SHARD_DATA, null, null, null, null, false, false));
        cachedDecisions.put(AllocationStatus.NO_VALID_SHARD_COPY,
            new AllocateUnassignedDecision(Type.NO, AllocationStatus.NO_VALID_SHARD_COPY, null, null, null, null, false, false));
        cachedDecisions.put(AllocationStatus.DECIDERS_NO,
            new AllocateUnassignedDecision(Type.NO, AllocationStatus.DECIDERS_NO, null, null, null, null, false, false));
        cachedDecisions.put(AllocationStatus.DECIDERS_THROTTLED,
            new AllocateUnassignedDecision(Type.THROTTLE, AllocationStatus.DECIDERS_THROTTLED, null, null, null, null, false, false));
        cachedDecisions.put(AllocationStatus.DELAYED_ALLOCATION,
            new AllocateUnassignedDecision(Type.NO, AllocationStatus.DELAYED_ALLOCATION, null, null, null, null, false, false));
    ...
    public static AllocateUnassignedDecision readFrom(StreamInput in) throws IOException {
        ...
        Decision shardDecision;
        if (in.readBoolean()) {
            shardDecision = Decision.readFrom(in);
        } else {
            shardDecision = null;
        }
        ...
        return new AllocateUnassignedDecision(finalDecision, allocationStatus, assignedNodeId, allocationId,
                                                 nodeDecisions, shardDecision, forceAllocated, reuseStore);
    }
    ...
    public void writeTo(StreamOutput out) throws IOException {
        ...
        if (shardDecision != null) {
            out.writeBoolean(true);
            Decision.writeTo(shardDecision, out);
        } else {
            out.writeBoolean(false);
        }
    ...
    /**
     * Returns a NO decision with the given shard-level decision.
     */
    public static AllocateUnassignedDecision no(Decision shardDecision, AllocationStatus allocationStatus) {
        return new AllocateUnassignedDecision(Type.NO, allocationStatus, null, null, null, shardDecision, false, false);
    }
    ...
    public static AllocateUnassignedDecision no(AllocationStatus allocationStatus, @Nullable Map<String, NodeAllocationResult> decisions,
                                                boolean reuseStore) {
        if (decisions != null) {
            return new AllocateUnassignedDecision(Type.NO, allocationStatus, null, null, decisions, null, false, reuseStore);
        } else {
    ...
    public static AllocateUnassignedDecision yes(String assignedNodeId, @Nullable String allocationId,
                                                 ...
                                                 boolean reuseStore) {
        return new AllocateUnassignedDecision(Type.YES, null, assignedNodeId, allocationId, decisions, null, forceAllocated, reuseStore);
    }
    ...
    public static AllocateUnassignedDecision fromDecision(Decision decision, @Nullable String assignedNodeId,
        ...
        return new AllocateUnassignedDecision(decisionType, allocationStatus, assignedNodeId, null, nodeDecisions, null, false, false);
    }
    ...
    /**
     * Returns the shard-level decision, without taking any nodes into account.  This value is
     * only populated in the case of a replica shard that can not be allocated without having
     * to check against each node (for example, a replica shard can never be allocated if the
    ...
    public String getFinalExplanation() {
        ...
        if (finalDecision == Type.NO) {
            ...
            if (shardDecision != null) {
                explanation = "shard cannot be allocated, irrespective of the nodes in the cluster";
    ...
    public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        if (shardDecision != null) {
            shardDecision.toXContent(builder, params);
        }
        if (nodeDecisions != null) {
