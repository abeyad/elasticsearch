...
public class AllocateUnassignedDecision implements ToXContent, Writeable {
     ...
     */
    public static AllocateUnassignedDecision no(AllocationStatus allocationStatus,
                                                @Nullable Map<String, NodeAllocationResult> nodeResults) {
        return no(allocationStatus, nodeResults, false);
    }
     ...
     * decisions that comprised the final NO decision if in explain mode.
     */
    public static AllocateUnassignedDecision no(AllocationStatus allocationStatus, @Nullable Map<String, NodeAllocationResult> nodeResults,
                                                boolean reuseStore) {
        Objects.requireNonNull(allocationStatus, "allocationStatus must not be null");
        if (nodeResults != null) {
            return new AllocateUnassignedDecision(Type.NO, allocationStatus, null, null, nodeResults, false, reuseStore);
        } else {
     ...
     */
    public static AllocateUnassignedDecision throttle(@Nullable Map<String, NodeAllocationResult> nodeResults) {
        if (nodeResults != null) {
            return new AllocateUnassignedDecision(Type.THROTTLE, AllocationStatus.DECIDERS_THROTTLED, null, null,
                                                     nodeResults, false, false);
        } else {
    ...
    public static AllocateUnassignedDecision yes(String assignedNodeId, @Nullable String allocationId,
                                                 @Nullable Map<String, NodeAllocationResult> nodeResults, boolean forceAllocated,
                                                 boolean reuseStore) {
        Objects.requireNonNull(assignedNodeId, "assigned node must not be null");
        return new AllocateUnassignedDecision(Type.YES, null, assignedNodeId, allocationId, nodeResults, forceAllocated, reuseStore);
    }
    ...
    public String getFinalExplanation() {
        ...
        if (finalDecision == Type.NO) {
            ...
            assert finalDecision == Type.THROTTLE;
