...
public abstract class PrimaryShardAllocator extends AbstractComponent {
    ...
    public void allocateUnassigned(RoutingAllocation allocation) {
        ...
        while (unassignedIterator.hasNext()) {
            ...
            final AllocationDecision allocationDecision = makeAllocationDecision(shard, allocation, false);

            if (allocationDecision.isDecisionTaken() == false) {
                // no decision was taken by this allocator
                continue;
            }

            if (allocationDecision.getDecision().type() == Type.YES) {
                unassignedIterator.initialize(allocationDecision.getAssignedNodeId(),
                    allocationDecision.getAllocationId(),
                    ShardRouting.UNAVAILABLE_EXPECTED_SHARD_SIZE,
                    allocation.changes());
            } else {
                unassignedIterator.removeAndIgnore(allocationDecision.getAllocationStatus(), allocation.changes());
            }
        }
    }

    /**
     * Is this allocator responsible for allocating the given {@link ShardRouting}?
     */
    private boolean isResponsibleFor(final ShardRouting shard) {
        return shard.primary() // must be primary
                && shard.unassigned() // must be unassigned
                && shard.recoverySource().getType() != RecoverySource.Type.EXISTING_STORE // only handles allocating existing shard copies
                && shard.recoverySource().getType() != RecoverySource.Type.SNAPSHOT; // BalancedShardsAllocator handles snapshot restores
    }

    /**
     * TODO: javadocs
     * TODO: this could be static, except for the logger used here and elsewhere, we don't want to lose
     * the valuable logging but it also doesn't make sense to log for the the explain API case - find a better solution here
     */
    public AllocationDecision makeAllocationDecision(final ShardRouting shard, final RoutingAllocation allocation, final boolean explain) {
        if (isResponsibleFor(shard) == false) {
            // this allocator is not responsible for allocating this shard
            return AllocationDecision.DECISION_NOT_TAKEN;
        }

        final AsyncShardFetch.FetchResult<NodeGatewayStartedShards> shardState = fetchData(shard, allocation);
        if (shardState.hasData() == false) {
            allocation.setHasPendingAsyncFetch();
            return AllocationDecision.noDecision(AllocationStatus.FETCHING_SHARD_DATA,
                "still fetching shard state from the nodes in the cluster");
        }

        // don't create a new IndexSetting object for every shard as this could cause a lot of garbage
        // on cluster restart if we allocate a boat load of shards
        final IndexMetaData indexMetaData = allocation.metaData().getIndexSafe(shard.index());
        ...
        if (inSyncAllocationIds.isEmpty()) {
            ...
            nodeShardsResult = buildVersionBasedNodeShardsResult(shard, snapshotRestore || recoverOnAnyNode,
                                                                 allocation.getIgnoreNodes(shard.shardId()), shardState);
            ...
            logger.debug("[{}][{}]: version-based allocation for pre-{} index found {} allocations of {}", shard.index(),
                         shard.id(), Version.V_5_0_0_alpha1, nodeShardsResult.allocationsFound, shard);
        } else {
            ...
            logger.debug("[{}][{}]: found {} allocation candidates of {} based on allocation ids: [{}]", shard.index(), shard.id(),
                         nodeShardsResult.orderedAllocationCandidates.size(), shard, inSyncAllocationIds);
        }
        ...
        if (enoughAllocationsFound == false) {
            if (recoverOnAnyNode) {
                ...
                logger.debug("[{}][{}]: not allocating, number_of_allocated_shards_found [{}]",
                             shard.index(), shard.id(), nodeShardsResult.allocationsFound);
                return AllocationDecision.noDecision(AllocationStatus.NO_VALID_SHARD_COPY,
        ...
        if (nodesToAllocate.yesNodeShards.isEmpty() == false) {
            ...
            logger.debug("[{}][{}]: allocating [{}] to [{}] on primary allocation",
                         shard.index(), shard.id(), shard, decidedNode.nodeShardState.getNode());
            final String nodeId = decidedNode.nodeShardState.getNode().getId();
            return AllocationDecision.yesDecision("the allocation deciders returned a YES decision to allocate to node [" + nodeId + "]",
            ...
            if (nodesToForceAllocate.yesNodeShards.isEmpty() == false) {
                ...
                return AllocationDecision.yesDecision("",
                ...
                return AllocationDecision.throttleDecision(AllocationStatus.DECIDERS_THROTTLED,
                ...
                return AllocationDecision.noDecision(AllocationStatus.DECIDERS_NO,
            ...
            logger.debug("[{}][{}]: throttling allocation [{}] to [{}] on primary allocation",
                         shard.index(), shard.id(), shard, nodesToAllocate.throttleNodeShards);
            return AllocationDecision.throttleDecision(AllocationStatus.DECIDERS_THROTTLED,
