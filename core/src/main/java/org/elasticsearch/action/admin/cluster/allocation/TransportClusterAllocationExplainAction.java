...
public class TransportClusterAllocationExplainAction
    ...
    private ClusterAllocationExplanation explainShard(ShardRouting shardRouting, RoutingAllocation allocation,
        ...
        allocation.setDebugDecision(includeYesDecisions ? DebugMode.ON : DebugMode.EXCLUDE_YES_DECISIONS);
        ...
        if (allocateDecision == AllocateUnassignedDecision.NOT_TAKEN) {
            try {
            ...
            shardDecision = new ShardAllocationDecision(allocateDecision, null, null);
        }

        return new ClusterAllocationExplanation(shardRouting.shardId(), shardRouting.primary(), shardRouting.currentNodeId(),
